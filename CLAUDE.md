...
6. **쉬운 말로 설명하기 (2026-06-13 결정)** — 사용자에게 설명할 때는 전문용어·약어를 그대로 쓰지 말고 비전문가가 이해할 수 있는 쉬운 단어로 풀어서 설명한다. 꼭 필요한 전문용어는 처음 나올 때 한 줄로 쉽게 뜻을 덧붙인다(예: "Ulcer Index(낙폭이 얼마나 깊고 오래갔는지 합친 고통 점수)"). 비유·일상어를 적극 활용하고, 결론을 먼저 말한 뒤 이유를 설명한다. 단, `docs/`·코드 주석 등 내부 문서는 정확성을 위해 전문용어를 유지해도 된다 — 이 규칙은 *사용자와의 대화*에 적용된다.
