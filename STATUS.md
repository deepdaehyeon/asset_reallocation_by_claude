# 트레이딩 상태 스냅샷

> 생성: **2026-08-11 09:45 KST** · 마지막 실행: 2026-08-11 03:02

> ⚠️ 자동 생성 파일. 수동 편집 금지 (`scripts/snapshot_state.py`).

## 레짐

| 항목 | 값 |
|---|---|
| 확정 레짐 | **Goldilocks** |
| 마지막 전환일 | 2026-08-03 |
| 신뢰도 | 63.92% |
| HMM 매핑 | unsupervised (실행 81회, legacy 폴백 0회) |

**blend 확률 분포**

| 레짐 | 확률 |
|---|---|
| Goldilocks | 79.4% |
| Slowdown | 10.7% |
| Reflation | 9.9% |

## 자산

| 항목 | 값 |
|---|---|
| 총자산 | ₩209,295,264 |
| 원금 | ₩211,575,378 |
| 누적 손익 | ₩-2,280,113 (-1.08%) |
| 고점 | ₩212,148,356 |
| 드로우다운 | -1.34% |
| S&P500였다면 | ₩207,735,460 |
| **알파(vs S&P500)** | **₩1,559,804 (+0.75%)** |
| 이번 달 회전액 | ₩45,368,406 (2026-08) |
| USD/KRW | 1,438.3 (2026-08-03 10:06) |

## 리밸런싱 트리거

| 계좌 | drift | 트리거 | 사유 | 마지막 리밸 |
|---|---|---|---|---|
| KRW | 3.17% | ⚪ | no_trigger(drift=3.2%) | 2026-08-06 10:02 |
| USD | 3.17% | ⚪ | no_trigger(drift=3.2%) | 2026-08-06 03:02 |

지연 매수: 없음

## 목표 비중 (블렌딩 결과)

| 자산군 | 비중 |
|---|---|
| equity_etf | 55.1% |
| equity_factor | 7.7% |
| gold | 6.7% |
| managed_futures | 5.7% |
| cash | 5.6% |
| commodity | 5.6% |
| equity_developed | 4.4% |
| equity_sector | 3.1% |
| bond_krw | 3.0% |
| equity_emerging | 2.0% |
| bond_tips | 1.2% |

## 매크로 피처 (마지막 실행)

| 지표 | 값 |
|---|---|
| 모멘텀 1M | 3.1% |
| 모멘텀 3M | 4.8% |
| 실현변동성 | 14.2% |
| VIX | 15.53 |
| VIX 기간구조 | -2.77 |
| 크레딧 신호 | 0.02 |
| HY 스프레드 | 1.61 |
| HY z-score | -0.31 |
| NFCI | -0.53 |
| DXY 1M | -0.01 |
| 원자재 1M | 0.05 |

## 핵심 설정값
> 전체 설정은 `trading/config.yaml` 참조.

| 설정 | 값 |
|---|---|
| drift 임계 | 5.0% |
| 리밸 쿨다운 | 0일 |
| 실행/월간 회전율 상한 | 0 / 0 (0=무제한) |
| 레짐 타이밍 소스 | rule |
| confirmation / cooldown | 1회 / 0일 |
| blend 평활 α | 0.5 |
| 신뢰도 산식 / 임계 | min / 0.2 |
| HMM 안정화 / deadband | True / 0.3 |
| HMM override / crisis 우선 | 0.5 / 0.4 |
| vol target (기본) / floor | 0.1 / 0.65 |
| 레짐별 target vol | Goldilocks 0.1625, Reflation 0.1375, Slowdown 0.1125, Stagflation 0.1, Crisis 0.075 |

