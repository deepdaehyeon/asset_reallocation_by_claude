# 트레이딩 상태 스냅샷

> 생성: **2026-09-15 10:00 KST** · 마지막 실행: 2026-09-15 03:02

> ⚠️ 자동 생성 파일. 수동 편집 금지 (`scripts/snapshot_state.py`).

## 레짐

| 항목 | 값 |
|---|---|
| 확정 레짐 | **Goldilocks** |
| 마지막 전환일 | 2026-08-03 |
| 신뢰도 | 19.59% |
| HMM 매핑 | unsupervised (실행 108회, legacy 폴백 0회) |

**blend 확률 분포**

| 레짐 | 확률 |
|---|---|
| Goldilocks | 76.1% |
| Reflation | 9.5% |
| Stagflation | 8.9% |
| Slowdown | 5.5% |

## 자산

| 항목 | 값 |
|---|---|
| 총자산 | ₩227,622,728 |
| 원금 | ₩236,672,768 |
| 누적 손익 | ₩-9,050,040 (-3.82%) |
| 고점 | ₩234,733,989 |
| 드로우다운 | -3.03% |
| S&P500였다면 | ₩226,611,497 |
| **알파(vs S&P500)** | **₩1,011,231 (+0.45%)** |
| 이번 달 회전액 | ₩72,648,817 (2026-09) |
| USD/KRW | 1,416.5 (2026-08-17 10:00) |

## 리밸런싱 트리거

| 계좌 | drift | 트리거 | 사유 | 마지막 리밸 |
|---|---|---|---|---|
| KRW | 3.31% | ⚪ | no_trigger(drift=3.3%) | 2026-09-14 10:00 |
| USD | 3.31% | ⚪ | no_trigger(drift=3.3%) | 2026-09-09 03:02 |

지연 매수: 없음

## 목표 비중 (블렌딩 결과)

| 자산군 | 비중 |
|---|---|
| equity_etf | 52.7% |
| equity_factor | 7.8% |
| gold | 7.2% |
| cash | 6.3% |
| commodity | 5.9% |
| managed_futures | 5.8% |
| equity_developed | 4.3% |
| equity_sector | 3.4% |
| bond_krw | 2.9% |
| equity_emerging | 2.0% |
| bond_tips | 1.8% |

## 매크로 피처 (마지막 실행)

| 지표 | 값 |
|---|---|
| 모멘텀 1M | -1.2% |
| 모멘텀 3M | 1.3% |
| 실현변동성 | 10.2% |
| VIX | 16.52 |
| VIX 기간구조 | -2.39 |
| 크레딧 신호 | 0.00 |
| HY 스프레드 | 1.51 |
| HY z-score | -1.02 |
| 10Y-2Y 커브 | 0.33 |
| CPI YoY | 3.71 |
| CPI MoM z | 0.66 |
| 실업률 3M 변화 | -0.20 |
| BEI 5Y | 2.40 |
| M2 YoY | 5.41 |
| Fed BS YoY | 2.04 |
| NFCI | -0.56 |
| DXY 1M | -0.01 |
| 원자재 1M | 0.10 |

## 핵심 설정값
> 전체 설정은 `trading/config.yaml` 참조.

| 설정 | 값 |
|---|---|
| drift 임계 | 5.0% |
| 리밸 쿨다운 | 0일 |
| 실행/월간 회전율 상한 | 0 / 0 (0=무제한) |
| 레짐 타이밍 소스 | rule |
| confirmation / cooldown | 1회 / 0일 |
| blend 평활 α | 0.5 |
| 신뢰도 산식 / 임계 | min / 0.2 |
| HMM 안정화 / deadband | True / 0.3 |
| HMM override / crisis 우선 | 0.5 / 0.4 |
| vol target (기본) / floor | 0.1 / 0.65 |
| 레짐별 target vol | Goldilocks 0.1625, Reflation 0.1375, Slowdown 0.1125, Stagflation 0.1, Crisis 0.075 |

