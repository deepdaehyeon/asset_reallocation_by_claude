# 트레이딩 상태 스냅샷

> 생성: **2026-08-12 03:02 KST** · 마지막 실행: 2026-08-12 03:02

> ⚠️ 자동 생성 파일. 수동 편집 금지 (`scripts/snapshot_state.py`).

## 레짐

| 항목 | 값 |
|---|---|
| 확정 레짐 | **Goldilocks** |
| 마지막 전환일 | 2026-08-03 |
| 신뢰도 | 49.90% |
| HMM 매핑 | unsupervised (실행 82회, legacy 폴백 0회) |

**blend 확률 분포**

| 레짐 | 확률 |
|---|---|
| Goldilocks | 81.2% |
| Reflation | 10.7% |
| Slowdown | 8.0% |

## 자산

| 항목 | 값 |
|---|---|
| 총자산 | ₩208,921,922 |
| 원금 | ₩211,252,841 |
| 누적 손익 | ₩-2,330,918 (-1.10%) |
| 고점 | ₩211,825,819 |
| 드로우다운 | -1.37% |
| S&P500였다면 | ₩206,167,872 |
| **알파(vs S&P500)** | **₩2,754,051 (+1.34%)** |
| 이번 달 회전액 | ₩45,368,406 (2026-08) |
| USD/KRW | 1,438.3 (2026-08-03 10:06) |

## 리밸런싱 트리거

| 계좌 | drift | 트리거 | 사유 | 마지막 리밸 |
|---|---|---|---|---|
| KRW | 3.68% | ⚪ | no_trigger(drift=3.7%) | 2026-08-06 10:02 |
| USD | 3.68% | ⚪ | no_trigger(drift=3.7%) | 2026-08-06 03:02 |

지연 매수: 없음

## 목표 비중 (블렌딩 결과)

| 자산군 | 비중 |
|---|---|
| equity_etf | 55.8% |
| equity_factor | 7.8% |
| gold | 6.4% |
| commodity | 5.8% |
| managed_futures | 5.8% |
| cash | 5.6% |
| equity_developed | 4.4% |
| equity_sector | 3.2% |
| bond_krw | 2.2% |
| equity_emerging | 2.0% |
| bond_tips | 1.1% |

## 매크로 피처 (마지막 실행)

| 지표 | 값 |
|---|---|
| 모멘텀 1M | 2.8% |
| 모멘텀 3M | 4.7% |
| 실현변동성 | 13.8% |
| VIX | 15.32 |
| VIX 기간구조 | -2.83 |
| 크레딧 신호 | 0.02 |
| 10Y-2Y 커브 | 0.47 |
| CPI YoY | 3.73 |
| CPI MoM z | -3.29 |
| 실업률 3M 변화 | -0.20 |
| Fed BS YoY | 1.62 |
| NFCI | -0.53 |
| DXY 1M | -0.01 |
| 원자재 1M | 0.05 |

## 핵심 설정값
> 전체 설정은 `trading/config.yaml` 참조.

| 설정 | 값 |
|---|---|
| drift 임계 | 5.0% |
| 리밸 쿨다운 | 0일 |
| 실행/월간 회전율 상한 | 0 / 0 (0=무제한) |
| 레짐 타이밍 소스 | rule |
| confirmation / cooldown | 1회 / 0일 |
| blend 평활 α | 0.5 |
| 신뢰도 산식 / 임계 | min / 0.2 |
| HMM 안정화 / deadband | True / 0.3 |
| HMM override / crisis 우선 | 0.5 / 0.4 |
| vol target (기본) / floor | 0.1 / 0.65 |
| 레짐별 target vol | Goldilocks 0.1625, Reflation 0.1375, Slowdown 0.1125, Stagflation 0.1, Crisis 0.075 |

