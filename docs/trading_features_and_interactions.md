...
| B1c | **모니터링 스케줄 1일 1회 통합** | `scripts/run_usd.sh`, launchd plist (리포 밖) | ✅ 2026-06-19 변경 | 화~토 05:00 KST(미장 마감 1h 전, 비서머타임) monitor→usd 체인 1회. 월 05:00은 monitor만(주말 trigger 새로고침). krw는 그대로 10:00, 그날 05:00 monitor 결과 사용 | 이전엔 09:30/23:00 두 번 모니터링해서 국장이 먼저 거래하면 그 효과가 23:00 재계산에 반영돼 미장 트리거가 깎이는 일이 가능했음(둘 다 같은 전체-drift를 쓰는데 평가 시점이 달라 생기는 불일치). 하루 1번으로 양쪽이 같은 스냅샷을 쓰게 통일. `trigger_max_age_hours`(기본 8h) 한도 내 유지가 핵심 제약 — 월요일 monitor-only가 그 안전장치 |
