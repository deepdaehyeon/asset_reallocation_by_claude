...
| A0-2 | **vol targeting** | `portfolio.py:168` | ✅ floor 0.65, 레짐별 목표 ×1.25 | 포트폴리오 EWMA 변동성이 레짐 목표(G0.1625~C0.075, 2026-06-17 전체 ×1.25 상향) 초과 시 equity를 비례 축소(floor까지), 축소분 cash | **A6 drawdown_scaling과 이중축소(끈 이유).** A1 core30이 위성 70%로 희석. A4 VIX캡과 같은 위기에 동시 작동. floor 결론이 B1 리밸 모드에 의존. **축소분 행선지 `reduction_dest`**(cash/defensive/nonequity) A/B/C 백테스트 결과 cash 유지(분배는 de-risk 약화로 위기낙폭·Ulcer 미세 악화, 차이 노이즈 [[experiment_2026-07-04_voltarget_reduction_dest]]) |
...
| A5 | **transition_phase** | `engine.py`, `regime_filter` | ⛔ days 0, **타깃 삭제 2026-07-01** | 레짐 전환 후 N일 Transition 비중(risk-off) | 켜면 blend·vol과 별개로 진입 비중 깎음(over-protection으로 보류). `regime_targets.Transition` 블록은 2026-07-01 제거 — 기계장치(transition_phase 인자·_check_transition)는 보존되나 `transition_days:0`이라 비활성. 되살리려면 config에 Transition 블록 복원 + days>0 (없으면 portfolio.py:34가 일반 blend로 폴백) |
...
| A6c | **reverse_synthetic (역합성)** | `portfolio.py` `derive_account_weights`, `config.reverse_synthetic` | ✅ enabled true (2026-07-04 신설) | KRW-native 목표가 KRW 계좌 여력 초과 시(방어+USD과잉) 등가 USD클래스(bond_krw→IEF/SHY·cash→SGOV·gold→GLD·equity_etf→SPY/QQQ)로 이동해 노출 보존, 매핑 없는 sector·tips는 축소 | forward 흡수(USD부족→KRW)의 **거울**. `usd_remaining>0`일 때만 발동 → bond_shortfall(USD부족)과 배타(순환 없음). 06-11 채권통합의 대칭 완성(IEF 재등장). SPY/QQQ/GLD universe 추가. 백테스트 usd_ratio=0.30 고정이라 미검증(라이브 전용 안전망). 현재 Goldilocks·USD37%라 미발동. [[feature_2026-07-04_reverse_synthetic]] |
...
| A17b | **soft_rule_scale (룰 소프트 라벨)** | `regime.py:detect_regime_soft`, `engine.py`, `hmm.soft_rule_scale` | ⛔ None (실험 후 불채택 2026-07-01) | RF(하드 룰 근사) 자리에 룰의 하드 임계를 로지스틱 램프로 소프트화한 확률분포 주입 | **불채택** — 소프트화가 4지표 단조 악화(rf0.70 OFF 회복 110→242일·Martin 1.87→1.75). RF가 정확한 하드룰보다도 우수. opt-in·off 보존. [[experiment_2026-07-01_soft_rule_label]] |
...
| # | 기능 | 위치 | 상태/값 | 하는 일 | 핵심 상호작용 |
|---|---|---|---|---|---|
| B1 | **drift_threshold (전체 포트폴리오 기준)** | `run.py:108,150` | ✅ 5.0%(2026-06-19 1.5%→5.0%) | 전체 자산 기준 drift>5%p면 트리거, KRW/USD 동일 값 사용 | 2026-06-19(`12f76ea`) 계좌별 재정규화 제거 — 분모가 작아 인위적으로 부풀려지던 drift를 백테스트와 같은 단일 전체 기준으로 통일. 같은 날 B1b 버그 수정 후 재스윕에서 0.5~15% 전 구간 위험이 거의 평평하고 5.0%가 Martin 최고+리밸 58%↓라 1.5%→5.0% 채택([[experiment_2026-06-19_drift_threshold_resweep3]]). **백테스트도 이 모드여야 함**(엔진 `_run_drift` vs `_run_calendar`). floor·vol 결론이 모드에 의존([[feedback-backtest-rebal-mode-drift]]) |
| B1b | **`backtest/engine.py::_run_drift` 일일 재평가 (기준선 이동)** | `backtest/engine.py` | ✅ 2026-06-19 수정 | 거래 여부와 무관하게 매일 레짐·블렌딩을 재계산해 drift를 잼(이전엔 거래일에만 목표 갱신 → 신호 드리프트 누락) | **`_run_drift` 쓰는 모든 과거 실험의 절대 수치(리밸 횟수·tx·Martin 등)가 이 수정으로 다시 이동한다** — ewma_lambda 수정([[experiment_2026-06-16_voltarget_lambda_sweep]])과 같은 종류의 "기준선 이동". 회귀검증: 2.5년 스모크테스트에서 drift=1.5% 트리거율 16%→48.5%로 급증([[experiment_2026-06-19_drift_threshold_resweep3]]) |
| B1c | **모니터링 스케줄 1일 1회 통합** | `scripts/run_usd.sh`, launchd plist (리포 밖) | ✅ 2026-06-19 변경, 2026-06-25 시각조정 | 화~토 **04:00 KST**(서머타임=마감 1h 전, 비서머타임=마감 2h 전) monitor→usd 체인 1회. 월 monitor만(주말 trigger 새로고침). krw는 그대로 10:00, 그날 04:00 monitor 결과 사용. ⚠️ 05:00은 서머타임에 마감 동시호가 순간(16:00 EDT=05:00 KST)에 걸려 DBC·DBMF 저유동 ETF 호가 0 스킵 발생 → 04:00으로 이동 | 이전엔 09:30/23:00 두 번 모니터링해서 국장이 먼저 거래하면 그 효과가 23:00 재계산에 반영돼 미장 트리거가 깎이는 일이 가능했음(둘 다 같은 전체-drift를 쓰는데 평가 시점이 달라 생기는 불일치). 하루 1번으로 양쪽이 같은 스냅샷을 쓰게 통일. `trigger_max_age_hours`(기본 8h) 한도 내 유지가 핵심 제약 — 월요일 monitor-only가 그 안전장치 |
