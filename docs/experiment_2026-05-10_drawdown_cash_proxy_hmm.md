...
> **요약**: ① drawdown 구간 equity 축소분을 현금성 자산으로 재배분하고, 261220 백테스트 프록시(USO)를 추가하며, HMM 수렴 안정성을 개선하는 세 가지 변경을 실험했다. ② COVID Crash 시 전략 MaxDD -10.55% vs 60/40 -19.13%, drift 5% 적용 시 CAGR 10.5%·비용 1.316%로 주간 대비 비용 80% 절감을 확인했다. ③ 세 변경 모두 채택되어 라이브에 반영됐으며, 261220 경고 스팸 및 HMM 수렴 로그가 제거됐다.
