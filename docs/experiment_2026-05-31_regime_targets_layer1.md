...
**C-v2 후보** — C안의 타당한 부분(MF 제거)은 유지하고 V-shape 기반 추가만 되돌리는
선별 조정안을 구성해 백테스트로 검증:
- Crisis: equity_etf 10→0 복원, bond_tips 10→축소 → cash/bond_usd/bond_krw로 환원
- Stagflation: equity_factor 8→3 복원, bond_krw 0→추가, gold 일부 유지
- Reflation: MF 12→축소 (단 commodity contemp #1 +2.85는 유지)

→ 코드(config) 변경이므로 프로젝트 규칙 3에 따라 사용자 확인 후 진행.
